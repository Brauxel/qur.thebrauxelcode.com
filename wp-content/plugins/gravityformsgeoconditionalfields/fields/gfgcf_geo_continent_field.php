...
class GF_Geo_Continent_Field extends GF_Field {

	public $type = 'geo_continent_field';

	public function get_form_editor_field_title() {
		return esc_attr__('Geo Continent', 'gf-geo-fields');
	}
	...
	public function get_form_editor_inline_script_on_page_render() {
		...
		<script type="text/javascript">
			function SetDefaultValues_geo_continent_field(field) {
	...
	public function get_display_value($value) {
		...
		$continents = gfgcf_controller::$instance->gfgcf_model->get_continents();
...
GF_Fields::register(new GF_Geo_Continent_Field);

class GF_Geo_Continent_Field_Extensions {
	...
	function gform_pre_render($form) {
		...
		foreach($form["fields"] as $index=>$field) {
			if($field["type"] == $this->type) {
				...
				$continents = gfgcf_controller::$instance->gfgcf_model->get_formatted_continents();
				$form["fields"][$index]["choices"] = $continents;

				$ip = $_SERVER["REMOTE_ADDR"];
				$results = gfgcf_controller::$instance->gfgcf_model->get_users_country_by_ip($ip);
				$form["fields"][$index]["defaultValue"] = $results[2];
			}
		}
	...
	function gform_pre_process($form) {
		foreach($form["fields"] as $index=>$field) {
			if($field["type"] == $this->type) {
				...
				$ip = $_SERVER["REMOTE_ADDR"];
				$results = gfgcf_controller::$instance->gfgcf_model->get_users_country_by_ip($ip);
				$_POST["input_" . $field["id"]] = $results[2];
			}
		}
	...
	function gform_field_css_class($classes, $field, $form){
		...
		if( $field->type == $this->type ){
			$classes .= ' gf_hidden gform_geo_continent_field';
		}
	...
	function gform_custom_merge_tags($merge_tags, $form_id, $fields, $element_id) {
		...
		if(in_array($element_id, $merge_locations)) {
			$merge_tags[] = array('tag' => '{geo_continent}', 'label' => __("Geo Continent Code", "gf-geo-fields"));
			$merge_tags[] = array('tag' => '{geo_continent_display}', 'label' => __("Geo Continent Display", "gf-geo-fields"));
		}
	...
	function replace_vars($text, $entry=false) {
		...
		if(is_array($text)) {
			...
			$pattern1 = '/\{geo_continent\}/i';
			$pattern2 = '/\{geo_continent_display\}/i';
			...
			if(!empty($match1) || !empty($match2)) {
				...
				if(!empty($ip)) {
					gfgcf_controller::$instance->model("gfgcf_model");
					$results = gfgcf_controller::$instance->gfgcf_model->get_users_country_by_ip($ip);
					$continent_code = $results[2];
					$continent_name = $results[3];
				} else {
					$continent_code = "";
					$continent_name = "";
				}
				$text = preg_replace($pattern1, $continent_code, $text);
				$text = preg_replace($pattern2, $continent_name, $text);
			}
		}
	...
	function ajax_field_added_load_options() {
		if(wp_verify_nonce($_POST["nonce"], 'gfgcf-load-field-options')) {
			...
			if($type == $this->type) {
				...
				$options = gfgcf_controller::$instance->gfgcf_model->get_formatted_continents();
...
new GF_Geo_Continent_Field_Extensions();
...
//	print_r($atts);
	...
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.6.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
	$out .= '$("#social-stream-'.$id.'.dc-feed").dcSocialStream(config);}});</script>'."\n";
	$out .= '<div id="social-stream-'.$id.'" class="dc-feed dc-feed-'.$style.'"></div>';
...
//	print_r($stream);
		...
		foreach($options['settings'] as $k => $v)
		{
			switch($k)
			{
				...
				if(isset($atts[$k])){
					...
					if($atts[$k] == 'classic'){
						$opt .= '' ;
					} else if($atts[$k] == 'modern-light'){
						$opt .= ',style: {layout: "modern",colour: "light"}' ;
					} else if($atts[$k] == 'modern-dark'){
						$opt .= ',style: {layout: "modern",colour: "dark"}' ;
					} else {
						$opt .= '' ;
					}
				} else {
					if($v != $settings[$k]){
						if($v == 'classic'){
							$opt .= '' ;
						} else if($v == 'modern-light'){
							$opt .= ',style: {layout: "modern",colour: "light"}' ;
						} else if($v == 'modern-dark'){
							$opt .= ',style: {layout: "modern",colour: "dark"}' ;
						} else {
							$opt .= '' ;
						}	
	...
	$out .='<script type="text/javascript">jQuery(document).ready(function($){';
	$out .= 'var config = '.$config.';';
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.wall.1.7.js", function(){}); $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.6.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
